...
#from megasdkrestclient.constants.State import TYPE_STATE_CANCELED, TYPE_STATE_FAILED, TYPE_STATE_COMPLETED
import asyncio
import os
...
class MegaDownloader():
        ...
        while True:
            ...
            if dl_info["state"] not in [constants.State.TYPE_STATE_CANCELED, constants.State.TYPE_STATE_FAILED]:
                ...
                if dl_info["state"] == constants.State.TYPE_STATE_CANCELED:
                    ...
                    await self._user_message.edit("Mega Cancelled")
                else:
                    self._is_errored = True
    ...
    async def create_update_message(self):
        ...
        msg = "<b>Downloading:</b> {}\n".format(update["name"])
        msg += "<b>Speed:</b> {}\n".format(human_readable_bytes(update["speed"]))
        msg += "<b>Progress:</b> {} - {}%\n".format(self.progress_bar((update["completed_length"]/update["total_length"])),
            round((update["completed_length"]/update["total_length"])*100, 2))
