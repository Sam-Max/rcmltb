...
class QbittorrentDownloader(BaseTask):
    def __init__(self, torrent, is_file = False):
        super().__init__()
    ...
    async def get_client(self, host=None,port=None,uname=None,passw=None, retry=None) -> qba.TorrentsAPIMixIn:
        ...
        try:
            ...
            self._client = client
            return client
        except qba.LoginFailed:
            LOGGER.exception("Invalid credentials detected")
            return None
        except qba.APIConnectionError:
            if retry == 0:
                LOGGER.error("Tried to get the client 3 times...")
                return None
            LOGGER.info("Qbittorrent server is not running. Attempting to start at port {}".format(port))
