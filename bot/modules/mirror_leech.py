...
async def mirror_leech(client, message, isLeech=False, sameDir=None):
    ...
    listener= MirrorLeechListener(message, tag, user_id, compress, extract, select, 
                                  seed, isLeech, sameDir)   
...
async def mirror_menu(client, query):
    ...
    if int(info[-2]) != user_id:
        ...
        if config_dict['REMOTE_SELECTION'] and not is_Leech:
            ...
            tgdown= TelegramDownloader(file, client, listener, f'{DOWNLOAD_DIR}{listener.uid}/')
            if PARALLEL_TASKS:    
                await m_queue.put(tgdown)
                return
            await tgdown.download() 
        ...
        await query_message.delete()
        question= await client.send_message(message.chat.id, text= "Send the new name with extension, /ignore to cancel")
        try:
            ...
            if response:
                if "/ignore" in response.text:
                    ...
                    if config_dict['REMOTE_SELECTION'] and not is_Leech:
                        ...
                        tgdown= TelegramDownloader(file, client, listener, f'{DOWNLOAD_DIR}{listener.uid}/', new_name) 
                        if PARALLEL_TASKS:    
                            await m_queue.put(tgdown)
                            return
                        await tgdown.download()  
...
bot.add_handler(MessageHandler(handle_mirror,filters=filters.command(BotCommands.MirrorCommand) & (CustomFilters.user_filter | CustomFilters.chat_filter)))
bot.add_handler(CallbackQueryHandler(mirror_menu, filters=filters.regex("mirrormenu")))
bot.add_handler(CallbackQueryHandler(mirror_select, filters=filters.regex("remoteselectmenu")))

if config_dict['AUTO_MIRROR']:
    bot.add_handler(MessageHandler(handle_auto_mirror, filters= filters.video | filters.document | filters.audio | filters.photo))
