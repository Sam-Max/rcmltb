...
from bot.helper.telegram_helper.message_utils import editMessage, sendFile, sendMessage, sendRss
...
async def rssSub(_, message, pre_event):
    ...
    for index, item in enumerate(items, start=1):
        args = item.split()
        if len(args) < 2:
            await sendMessage(f'{item}. Wrong Input format. Read help message before adding new subcription!', message)
            continue
        title = args[0].strip()
        if (user_feeds := rss_dict.get(user_id, False)) and title in user_feeds:
            await sendMessage(f"This title {title} already subscribed! Choose another title!", message)
            continue
...
async def rssUpdate(client, message, pre_event, state):
    ...
    await sendMessage(f"Rss links with Title(s): <code>{updated}</code> has been {state}d!", message)
...
async def rssGet(client, message, pre_event):
    ...
    if len(args) < 2:
        await sendMessage(f'{args}. Wrong Input format. You should add number of the items you want to get. Read help message before adding new subcription!', message)
        await updateRssMenu(pre_event)
        return
    try:
        title = args[0]
...
async def rssEdit(client, message, pre_event):
    ...
    for item in items:
        ...
        title = args[0].strip()
        if len(args) < 2:
            await sendMessage(f'{item}. Wrong Input format. Read help message before editing!', message)
            continue
    ...
    if int(data[2]) != user_id and not await CustomFilters.sudo_filter(client, query):
        ...
        if len(rss_dict.get(int(data[2]), {})) == 0:
            ...
            if data[1] == 'pause':
                ...
                buttons.cb_buildbutton("Resume AllMyFeeds",
...
bot.add_handler(MessageHandler(getRssMenu, filters=command(BotCommands.RssCommand) & CustomFilters.user_filter))
