...
async def rssSub(_, message, pre_event):
    ...
    for index, item in enumerate(items, start=1):
        ...
        if len(args) > 2:
            arg = item.split(" c: ", 1)
            cmd = (
                re_split(" inf: | exf: | opt: ", arg[1])[0].strip()
                if len(arg) > 1
                else None
            )
            arg = item.split(" inf: ", 1)
            inf = (
                re_split(" c: | exf: | opt: ", arg[1])[0].strip()
                if len(arg) > 1
                else None
            )
            arg = item.split(" exf: ", 1)
            exf = (
                re_split(" c: | inf: | opt: ", arg[1])[0].strip()
                if len(arg) > 1
                else None
            )
            arg = item.split(" opt: ", 1)
            opt = (
                re_split(" c: | inf: | exf: ", arg[1])[0].strip()
                if len(arg) > 1
                else None
            )
            if inf is not None:
...
async def rssEdit(client, message, pre_event):
    ...
    for item in items:
        ...
        arg = item.split(" inf: ", 1)
        inf = (
            re_split(" c: | exf: | opt: ", arg[1])[0].strip() if len(arg) > 1 else None
        )
        arg = item.split(" exf: ", 1)
        exf = (
            re_split(" c: | inf: | opt: ", arg[1])[0].strip() if len(arg) > 1 else None
        )
        arg = item.split(" opt: ", 1)
        opt = (
            re_split(" c: | inf: | exf: ", arg[1])[0].strip() if len(arg) > 1 else None
        )
        async with rss_dict_lock:
            if opt is not None:
                if opt.lower() == "none":
...
async def rssListener(client, query):
    ...
    if int(data[2]) != user_id and not await CustomFilters.sudo_filter(client, query):
        ...
        if len(rss_dict.get(int(data[2]), {})) == 0:
            ...
            button = buttons.build_menu(2)
            await editMessage(
                "Send one title with vlaue separated by space get last X items.\nTitle Value\nTimeout: 60 sec.",
                message,
                button,
            )
