                ...
                if len(lines) > 1:
                    ...
                    for link in lines:
                        ...
                        if len(args) > 1:
                            ...
                            auth = False
                        if auth:
                            msg= f"/leech {args[0]} -au {username} -ap {password}" if isLeech else f"/mirror {args[0]} -au {username} -ap {password}"
                        else:
                            msg= f"/leech {link}" if isLeech else f"/mirror {link}"  
                        if isLeech:
                            msg= await bot.send_message(message.chat.id, msg, disable_web_page_preview=True)
                        else:
                            msg= await bot.send_message(message.chat.id, msg, disable_web_page_preview=True)
