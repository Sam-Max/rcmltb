...
async def leech(client, message):
    ...
    if await is_rclone_config(user_id, message, isLeech=True):
        ...
        if message.reply_to_message:
            ...
            await sendMarkup("Select from where you want to leech", message, button.build_menu(2))  
        else:
...
async def leech_menu_cb(_, callback_query):
    ...
    if int(cmd[-1]) != user_id:
        ...
        if await is_valid_path(rclone_remote, base_dir, message):
            ...
            await list_folder(message, rclone_remote, base_dir, menu_type=Menus.LEECH, edit=True)
...
bot.add_handler(MessageHandler(handle_leech, filters= command(BotCommands.LeechCommand) & (CustomFilters.user_filter | CustomFilters.chat_filter)))
