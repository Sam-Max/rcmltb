    ...
    if method.startswith('api'):
        ...
        try:
            ...
            if 'error' in search_results or search_results['total'] == 0:
                await editMessage(f"No result found for <i>{key}</i>\nTorrent Site:- <i>{SITES.get(site)}</i>", message)
                return
        ...
        while True:
            ...
            if status != 'Running':
                break
        dict_search_results = await run_sync(client.search_results, search_id=search_id, limit=TELEGRAPH_LIMIT)
...
bot.add_handler(MessageHandler(torrentSearch, filters=command(BotCommands.SearchCommand)))
