...
async def handle_copy(client, message):
    ...
    if await is_rclone_config(user_id, message):
        ...
        if config_dict['MULTI_RCLONE_CONFIG'] or CustomFilters._owner_query(user_id): 
            await list_remotes(message, menu_type='copymenu', remote_type='remote_origin')
        else:
...
async def copy_menu_callback(client, callback_query):
    ...
    #Second Menu
        ...
        if cmd[3] == "True":
            ...
            origin_dir_= origin_dir + path  
            update_rclone_data("COPY_ORIGIN_DIR", origin_dir_, user_id)
        await list_remotes(message, menu_type='copymenu', remote_type="remote_dest", is_second_menu=True, edit=True)   
        await query.answer()   
    ...
    if cmd[1] == "back_origin":
        if len(origin_dir) == 0:
            await query.answer() 
            await list_remotes(message, menu_type='copymenu', remote_type="remote_origin", edit=True) 
            return
