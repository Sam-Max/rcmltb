...
class TelegramStatus:
      ...
      async def progress(self, current, total, name, status, current_time, inlineKeyboard):
            now = time.time()
            ...
            if round(diff % 10.00) == 0 or current == total:
                  ...
                  self._status_msg = self.get_status_message(current, total, speed, estimated_total_time, percentage, name, status)
                  await editMarkup(self._status_msg, self._message, reply_markup= inlineKeyboard)

      def get_status_message(self, current, total, speed, estimated_total_time, percentage, name, status):
            progress = "{0}{1}\n**P:** {2}%".format(
