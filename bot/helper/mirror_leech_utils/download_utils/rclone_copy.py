...
class RcloneCopy:
    ...
    async def copy(self, origin_remote, origin_dir, dest_remote, dest_dir):
                ...
                if SERVICE_ACCOUNTS_REMOTE:= config_dict['SERVICE_ACCOUNTS_REMOTE']:
                    if SERVICE_ACCOUNTS_REMOTE in config.sections():
                        if id := config[SERVICE_ACCOUNTS_REMOTE]['team_drive']:
                            ...
                            LOGGER.info(f'Using service account remote {SERVICE_ACCOUNTS_REMOTE}')
                    else:
        ...
        if return_code == 0:
            await self.__listener.onRcloneCopyComplete(conf_path, origin_dir, dest_remote, dest_dir, self.__isGdrive)
        else:
    ...
    def __create_teamdrive_sa_config(self, conf_path, id):
        ...
        rc_content += f"service_account_file = accounts/{self.__service_account_index}.json\n"
