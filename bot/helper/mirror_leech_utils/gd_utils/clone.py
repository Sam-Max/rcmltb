...
class gdClone(GoogleDriveHelper):
    ...
    def user_setting(self):
        if self.link.startswith("mtp:"):
            self.token_path = f"tokens/{self.listener.user_id}.pickle"
            self.upDest = self.link.lstrip("mtp:")
            self.use_sa = False
        elif self.link.startswith("tp:"):
            self.upDest = self.link.lstrip("tp:")
            self.use_sa = False
        elif self.link.startswith("sa:"):
            self.upDest = self.link.lstrip("sa:")
            self.use_sa = True

    def clone(self):
        try:
            file_id = self.getIdFromUrl(self.link)
        ...
        try:
            ...
            if mime_type == self.G_DRIVE_DIR_MIME_TYPE:
                dir_id = self.create_directory(meta.get("name"), self.upDest)
                ...
                file = self._copyFile(meta.get("id"), self.upDest)
            ...
            if "User rate limit exceeded" in err:
                ...
                msg = f"Error.\n{err}"
            run_async_to_sync(self.listener.onUploadError, msg)
            return None, None, None, None, None, None
