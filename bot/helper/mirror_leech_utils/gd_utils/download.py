...
class gdDownload(GoogleDriveHelper):
    def __init__(self, listener, name, link, path):
    ...
    def download(self):
        file_id = self.getIdFromUrl(self.link, self.listener.user_id)
        self.service = self.authorize()
        self._updater = setInterval(self.update_interval, self.progress)
        try:
            meta = self.getFileMetadata(file_id)
            if meta.get("mimeType") == self.G_DRIVE_DIR_MIME_TYPE:
                self._download_folder(file_id, self._path, self.name)
            else:
                makedirs(self._path, exist_ok=True)
                self._download_file(
                    file_id, self._path, self.name, meta.get("mimeType")
                )
            ...
            if "downloadQuotaExceeded" in err:
                ...
                err = "File not found!"
            run_async_to_sync(self.listener.onDownloadError, err)
            self.is_cancelled = True
        finally:
            self._updater.cancel()
            if self.is_cancelled:
                return
            run_async_to_sync(self.listener.onDownloadComplete)
